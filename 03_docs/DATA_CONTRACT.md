...
최대 context를 `C`라고 할 때 원문 길이가 정확히 `C` 또는 `2C`이면 EOS를 붙인 뒤 EOS 하나만 든 마지막 조각이 생긴다. 이 조각에는 앞 token으로 다음 token을 예측하는 causal target이 없으므로 제외한다. 원문 길이가 `C-1` 또는 `C+1`이면 EOS 앞에 원문 token이 함께 있어 마지막 token에서 EOS로 가는 target을 학습한다. 이 규칙은 겹치지 않는 고정 길이 parent 계약을 유지하기 위한 의도된 경계 동작이다.

정확한 `C` 배수 문서가 드물 것이라는 판단은 아직 C4 전체에서 측정하지 않은 가정이다. 다만 서로 다른 parent는 model의 서로 다른 forward 입력이며 KV cache나 recurrent state를 이어 쓰지 않으므로, EOS target이 없는 정확 배수 사례가 모든 문서를 하나의 연속 문서로 연결하지는 않는다.
...
## Streaming과 원본 local shard

Train은 shard 순서만 seed로 섞는다. Row shuffle buffer는 쓰지 않는다. Hugging Face Datasets의 shuffle buffer 내용은 iterator state에 저장되지 않으므로 exact resume과 양립하지 않는다. Validation은 섞지 않는다.
...
`local_raw`는 원본 C4 gzip shard를 직접 읽는다. Train shard 순서만 seed로 섞고 validation은 파일명 정렬 순서를 유지한다. 파일 목록 또는 크기가 바뀌면 source manifest SHA-256이 달라져 native resume을 거부한다.
